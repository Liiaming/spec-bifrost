...
function validateComponent(component: ComponentSpec, path: string, errors: SpecDiagnostic[], pageContext: string | undefined): void {
  ...
  validateOptionalArray(component.fields, `${path}.fields`, "fields", errors, componentContext)?.forEach((field, fieldIndex) =>
    validateField(field, `${path}.fields[${fieldIndex}]`, errors, componentContext)
  );
  validateOptionalArray(component.columns, `${path}.columns`, "columns", errors, componentContext)?.forEach((field, fieldIndex) =>
    validateField(field, `${path}.columns[${fieldIndex}]`, errors, componentContext)
  );
  validateOptionalArray(component.actions, `${path}.actions`, "actions", errors, componentContext)?.forEach((action, actionIndex) =>
...
function validateAction(action: ActionSpec, path: string, errors: SpecDiagnostic[], componentContext: string | undefined): void {
  if (!isRecord(action)) {
...
function validateCondition(condition: ConditionSpec | undefined, path: string, errors: SpecDiagnostic[], context?: string): void {
  ...
  if (Array.isArray(all)) {
    all.forEach((child, index) => validateCondition(child as ConditionSpec, `${path}.all[${index}]`, errors, context));
  } else if (all !== undefined) {
    errors.push(error("schema_error", `${path}.all`, "all must be an array.", all, context));
  }
  const any = record["any"];
  if (Array.isArray(any)) {
    any.forEach((child, index) => validateCondition(child as ConditionSpec, `${path}.any[${index}]`, errors, context));
  } else if (any !== undefined) {
    errors.push(error("schema_error", `${path}.any`, "any must be an array.", any, context));
  }
}

function validateNavigation(nav: unknown, path: string, errors: SpecDiagnostic[], context?: string): void {
          ...
          component.actions.forEach((action, actionIndex) => {
            ...
            if (typeof targetPageId === "string" && !pageIds.has(targetPageId)) {
              errors.push(
                error(
                  "reference_error",
                  `${actionPath}.targetPageId`,
                  `targetPageId "${targetPageId}" does not match any page id.`,
                  targetPageId,
                  actionContext
                )
              );
  ...
  spec.pages.forEach((page, pageIndex) => {
    ...
    page.sections.forEach((section, sectionIndex) => {
      ...
      section.components.forEach((component, componentIndex) => {
        ...
        const componentPath = `pages[${pageIndex}].sections[${sectionIndex}].components[${componentIndex}]`;
        const componentContext = joinContext(contextPart("page", page["id"]), contextPart("component", component["id"]));
        collectUniqueFieldIds(component["fields"] as FieldSpec[] | undefined, `${componentPath}.fields`, fieldIds, errors, componentContext);
        collectUniqueFieldIds(component["columns"] as FieldSpec[] | undefined, `${componentPath}.columns`, fieldIds, errors, componentContext);
      });
    });
  });
}
...
function validateConditionReferences(condition: ConditionSpec | undefined, path: string, fieldIds: Set<string>, errors: SpecDiagnostic[], context?: string): void {
  if (condition === undefined) return;
...
function validateEmptyState(emptyState: unknown, path: string, errors: SpecDiagnostic[], context?: string): void {
...
function validateOptionalString(value: unknown, path: string, errors: SpecDiagnostic[], context?: string): void {
  if (value === undefined) return;
  requireString(value, path, errors, context);
}

function validateOptionalBoolean(value: unknown, path: string, errors: SpecDiagnostic[], context?: string): void {
  if (value === undefined) return;
...
function validateStringArray(value: unknown, path: string, label: string, errors: SpecDiagnostic[], context?: string): void {
  if (value === undefined) return;
...
function validateOptionValues(value: OptionValue[] | undefined, path: string, errors: SpecDiagnostic[], required = false, context?: string): void {
  if (value === undefined) {
    if (required) errors.push(error("schema_error", path, "options must be an array.", value, context));
    return;
  }
...
function validateOptionalArray<T>(value: T[] | undefined, path: string, label: string, errors: SpecDiagnostic[], context?: string): T[] | undefined {
...
function requireString(value: unknown, path: string, errors: SpecDiagnostic[], context?: string): void {
...
function error(type: SpecDiagnostic["type"], path: string, message: string, value: unknown, context?: string): SpecDiagnostic {
...
function contextPart(kind: string, value: unknown): string | undefined {
...
function joinContext(...parts: Array<string | undefined>): string | undefined {
  const definedParts = parts.filter((part): part is string => part !== undefined);
