...
export function renderPrototypeHtml(input) {
  ...
  <div class="app-frame">
    ...
    <main class="workspace" id="main-content">
      <div class="topbar">
        ...
        <button type="button" class="ghost-button" data-notes-toggle aria-pressed="false">显示备注</button>
      </div>
...
function renderNavigation(navPages, firstPageId) {
    ...
    return `<nav class="nav">${Array.from(groups.entries())
      ...
      <section class="nav-group">
        ...
        ${pages
        .map((page) => `<button type="button" class="${page.id === firstPageId ? "active" : ""}" data-page-id="${escapeHtml(page.id)}"${page.id === firstPageId ? ' aria-current="page"' : ""}>
...
function renderComponentTitle(component) {
    if (!component.title && (!component.notes || component.notes.length === 0))
        return "";
...
function renderFilterBar(component, context) {
...
function renderTable(component, context) {
...
function renderTableRows(component, columns, rowActions, context) {
    if (columns.length === 0)
        return "";
...
function renderEditableTable(component, context) {
    const columns = component.columns ?? component.fields ?? [];
    return `
...
function renderEditableTableRows(component, columns, context) {
    if (columns.length === 0)
        return "";
...
function renderTreeTable(component, context) {
    const columns = component.columns ?? component.fields ?? [];
    const records = normalizeRecords(component.items);
    return `
    ${renderComponentTitle(component)}
...
function renderTreeTableRows(records, columns, context, depth = 0) {
    return records
        .map((record) => {
...
function renderComparisonTable(component, context) {
...
function normalizeRecords(items) {
    if (!items)
        return [];
    return items.filter((item) => Boolean(item) && typeof item === "object" && !Array.isArray(item));
}
function sampleRecord(columns, index, context) {
    return Object.fromEntries(columns.map((column) => [column.id, sampleValue(column, index, context)]));
}
function renderForm(component, context) {
    const fields = component.fields ?? [];
    return `
    ${renderComponentTitle(component)}
...
function renderDescriptionList(component, context) {
    const fields = component.fields ?? [];
    return `
    ${renderComponentTitle(component)}
...
function renderSteps(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0)
        return `${renderComponentTitle(component)}${renderEmptyState(component.emptyState ?? { title: "暂无进度记录" })}`;
    return `
    ${renderComponentTitle(component)}
    <ol class="steps-list">
      ${records
...
function renderMetricList(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0)
        return `${renderComponentTitle(component)}${renderEmptyState(component.emptyState ?? { title: "暂无指标配置" })}`;
    return `
    ${renderComponentTitle(component)}
...
function renderTabs(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0)
        return `${renderComponentTitle(component)}${renderEmptyState(component.emptyState ?? { title: "暂无分组内容" })}`;
    return `
    ${renderComponentTitle(component)}
...
function renderModal(component, context) {
    const fields = component.fields ?? [];
    return `
...
function renderDrawer(component, context) {
    const fields = component.fields ?? [];
    return `
...
function renderKanbanBoard(component) {
...
function renderWorkflowDiagram(component) {
...
function renderWizard(component) {
...
function renderProgressTracker(component) {
    const records = normalizeRecords(component.items);
    return `
    ${renderComponentTitle(component)}
...
function renderResultPanel(component) {
    return `
...
function renderGantt(component) {
    const records = normalizeRecords(component.items);
    return `
    ${renderComponentTitle(component)}
...
function renderTimeline(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0)
        return `${renderComponentTitle(component)}${renderEmptyState(component.emptyState ?? { title: "暂无时间线记录" })}`;
    return `
    ${renderComponentTitle(component)}
...
function renderTreeList(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0)
        return `${renderComponentTitle(component)}${renderEmptyState(component.emptyState ?? { title: "暂无层级数据" })}`;
    return `
    ${renderComponentTitle(component)}
    ${renderTreeNodes(records, true)}
  `;
}
function renderTreeNodes(records, isRoot = false) {
...
function renderTableBodyOnly(component, context, emptyTitle) {
    ...
    if (columns.length === 0 || records.length === 0)
        return renderEmptyState(component.emptyState ?? { title: emptyTitle });
...
function renderRuleList(component) {
    const records = normalizeRecords(component.items);
...
function renderAttachmentList(component) {
    const records = normalizeRecords(component.items);
...
function renderCommentThread(component) {
    const records = normalizeRecords(component.items);
...
function renderOrgChart(component) {
    const records = normalizeRecords(component.items);
...
function renderCollapsePanel(component) {
    const records = normalizeRecords(component.items);
...
function renderRelationGraph(component) {
    const records = normalizeRecords(component.items);
...
function renderCardList(component) {
    const records = normalizeRecords(component.items);
    if (records.length === 0) {
...
function renderAlert(component) {
...
function renderSimpleComponent(component, context) {
...
function renderFieldControl(field, context, mode) {
...
function renderActionButton(action, tone) {
...
function isRowAction(action) {
    if (/batch|bulk/i.test(action.id) || /批量/.test(action.label))
        return false;
    return /view|detail|edit|approve/i.test(action.id) || /查看|详情|编辑|处理|审批/.test(action.label);
}
function renderTableCell(value, field, context) {
...
function sampleValue(field, index, context) {
    const options = getOptions(field, context);
    if (options[0])
        return options[Math.min(index, options.length - 1)]?.label ?? options[0].label;
    if (/no|code|编号/i.test(field.id))
        return `REQ-2026-${String(index + 1).padStart(3, "0")}`;
    if (field.type === "currency")
        return ["¥12,800.00", "¥48,600.00", "¥6,200.00"][index] ?? "¥9,800.00";
    if (field.type === "number")
        return String([8, 16, 24][index] ?? 1);
    if (field.type === "date" || field.type === "dateRange")
        return "2026-05-26";
    if (field.type === "user")
        return ["李明", "周然", "陈悦"][index] ?? "李明";
    if (field.type === "department")
        return ["行政部", "研发部", "财务部"][index] ?? "行政部";
    if (/title|name|名称|标题/i.test(field.id + field.label))
        return `${field.label}示例 ${index + 1}`;
    return "待填写";
}
...
function renderRelationList(component) {
    if (!component.relations || component.relations.length === 0)
        return "";
...
:root {
  ...
  --line-strong: #c8d1de;
  --accent: #315aa6;
  --accent-strong: #244681;
  --accent-soft: #edf4ff;
  --accent-tint: #f6f9ff;
  --attention: #9a5b12;
  --attention-soft: #fff7e8;
  --success: #17735f;
  --success-soft: #eaf7f2;
  --warn-bg: #fff7e8;
  --warn-line: #d58a23;
...
body {
  ...
  font-size: 14px;
  line-height: 1.5;
  -webkit-tap-highlight-color: rgba(49, 90, 166, 0.14);
}
...
button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible {
  outline: 2px solid rgba(49, 90, 166, 0.46);
  outline-offset: 2px;
}
.skip-link {
  position: fixed;
...
.brand-mark {
  ...
  place-items: center;
  border-radius: 8px;
  border: 1px solid rgba(49, 90, 166, 0.24);
...
.nav button.active {
  border-color: rgba(49, 90, 166, 0.28);
  background: var(--accent-soft);
  color: var(--accent-strong);
  box-shadow: inset 0 0 0 1px rgba(49, 90, 166, 0.08);
}
.nav-page-label {
  display: block;
...
.ghost-button:hover, .secondary-button:hover { border-color: rgba(49, 90, 166, 0.44); color: var(--accent-strong); background: #f8fafc; }
.ghost-button:active, .secondary-button:active, .primary-button:active, .text-button:active { transform: translateY(1px); }
.primary-button {
  border: 1px solid var(--accent);
  background: var(--accent);
  color: #ffffff;
}
.primary-button:hover { background: var(--accent-strong); border-color: var(--accent-strong); box-shadow: 0 1px 2px rgba(23, 32, 51, 0.14); }
.text-button {
  border: 0;
  background: transparent;
  color: var(--accent);
...
input:focus, select:focus, textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(49, 90, 166, 0.14); }
.field-help {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
...
.wizard-preview li.active { border-color: rgba(49, 90, 166, 0.32); background: var(--accent-tint); }
...
.gantt-row i { display: block; height: 100%; border-radius: inherit; background: rgba(49, 90, 166, 0.76); }
...
@media (max-width: 900px) {
  ...
  .brand { grid-template-columns: 40px 1fr; gap: 10px; }
  .brand-mark { width: 40px; height: 40px; }
  .brand-copy span { -webkit-line-clamp: 1; }
  .nav { gap: 12px; }
  .nav-group { flex-direction: row; flex-wrap: wrap; }
  .nav-group-title { width: 100%; padding: 0; }
  .nav button {
    width: auto;
    min-height: 44px;
    padding: 7px 10px;
  }
  .ghost-button, .secondary-button, .primary-button, .text-button, input, select, textarea {
    min-height: 44px;
  }
  .sidebar-footer { padding: 12px 0 0; }
...
function renderConditionAttribute(name, condition) {
    if (!condition)
        return "";
